...
it("has a route handler listening tgo /api/tickets for post requests", async () => {
...
it("creates a ticket with valid inputs", async () => {
  // TODO: add a check to ensure that the ticket was saved into the database.
  let tickets = await Ticket.find({});
  expect(tickets.length).toEqual(0);

  let ticketTitle = "test ticket";
  let ticketPrice = 20;
  await request(app)
    .post("/api/tickets")
    .set("Cookie", global.signin())
